...
class Node:
    ...
    def message_consumer(self, in_link):
        ...
        while True:
            ...
            if ((mid, nid)) in self.recieved_id_cache:
                ...
                #print("input buffer at node", self.iden, ":", len(self.input_buffer
                    # [(roundn, stepn)]), " round:", roundn, " step: ", stepn)
    ...
    def message_generator(self, env):
        while True:
            ...
            if self.iden < NODE_COUNT * ADVERSARY_FRACTION:
                self.prev_block = proposed_block
                self.prev_block.state = "final"
                self.blockchain.append(proposed_block)
                print("Adversary", self.iden, "added empty block to its blockchain.", proposed_block.s)
...
class Message:
    ...
    def __init__(self, node, payload, m_type, roundn, stepn):
        ...
        self.message_string = str(self.payload) + "<|>" + str(self.publickey) + "<|>" +\
                str(self.msg_id) + "<|>" + str(self.node_id) + "<|>" + str(self.m_type) \
                + "<|>" + str(roundn) + "<|>" + str(stepn)
...
for i in range(NODE_COUNT):
    ...
    for j in range(len(node_conn_matrix[i])):
        ...
        """Instantiate a link, return it to get_output_conn, get_output_conn will return the 
        pipe which is given to the message_consumer
